...
class Test_1a(GradedTestCase):
    def setUp(self):
        ...
        self.multitask_net = submission.MultiTaskNet
        self.sol_multitask_net = self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol.MultiTaskNet)
    ...
    def test_2(self):
        ...
        A, B = self.multitask_net.init_user_and_item_bias(self, self.num_users, self.num_items)
        if A == None or B == None:
            self.assertTrue(False, 'init_user_and_item_bias not implemented')
        self.assertTrue(A.weight.shape == torch.Size([self.num_users, 1]), 'incorrect tensor shape for self.A')
...
class Test_2(GradedTestCase):
    def setUp(self):
        ...
        self.multitask_net_with_sharing = submission.MultiTaskNet(944, 1683, 32, [96, 64], True)
        self.multitask_net_without_sharing = submission.MultiTaskNet(944, 1683, 32, [96, 64], False)
