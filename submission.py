...
class MultiTaskNet(nn.Module):
    ...
    def forward_with_embedding_sharing(self, user_ids, item_ids):
        ...
        ### START CODE HERE ###
        user_embedded = self.U(user_ids)
        item_embedded = self.Q(item_ids)
        user_bias = self.A(user_ids)
        item_bias = self.B(item_ids)

        predictions = torch.mul(user_embedded, item_embedded).sum(dim=1) + user_bias.squeeze() + item_bias.squeeze()

        product = user_embedded * item_embedded
        concatenated_embeddings = torch.cat([user_embedded, item_embedded, product], dim=1)
        for layer in self.mlp_layers:
            concatenated_embeddings = layer(concatenated_embeddings)

        score = concatenated_embeddings.squeeze(dim=1)
        ### END CODE HERE ###
        return predictions, score
    ...
    def forward_without_embedding_sharing(self, user_ids, item_ids):
        ...
        predictions = score = None
        ### START CODE HERE ###
        user_reg_embedded = self.U_reg(user_ids)
        item_reg_embedded = self.Q_reg(item_ids)
        user_fact_embedded = self.U_fact(user_ids)
        item_fact_embedded = self.Q_fact(item_ids)
        user_bias = self.A(user_ids)
        item_bias = self.B(item_ids)

        predictions = torch.mul(user_fact_embedded, item_fact_embedded).sum(dim=1) + user_bias.squeeze() + item_bias.squeeze()

        product = user_reg_embedded * item_reg_embedded
        concatenated_embeddings = torch.cat([user_reg_embedded, item_reg_embedded, product], dim=1)
        for layer in self.mlp_layers:
            concatenated_embeddings = layer(concatenated_embeddings)
