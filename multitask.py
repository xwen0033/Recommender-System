...
class MultitaskModel(object):
    ...
    def __init__(self,
        ...
        self._random_state = random_state or np.random.RandomState(seed=123)
    ...
    def fit(self, interactions):
        ...
        for _ in range(self._n_iter):
            ...
            users = utils.gpu(torch.from_numpy(users),
                                        self._use_gpu)
            items = utils.gpu(torch.from_numpy(items),
                                        self._use_gpu)
            ratings = utils.gpu(torch.from_numpy(ratings),
                                       self._use_gpu)

            positive_prediction, score = self._net(users, items)
            negative_prediction = self._get_negative_prediction(users)
            ...
            regression_loss = self._regression_loss_func(ratings,
                                                            score)
